...
def api_access():
    ...
    if not user_info.get("fingerprint"):
        ...
        invite = create_one_time_link()

        return jsonify({"ok": True, "invite_link": invite})
    ...
    if user_info["fingerprint"] != current_fp:
        ...
        bot.send_message(user_id, f"🔐 OTP:\n`{otp}`")
        return jsonify({"ok": False, "need_otp": True, "message": "OTP Required"})
