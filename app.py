...
import json, os, hashlib, random, threading, time
import telebot

# ========= الإعدادات =========
BOT_TOKEN  = "8064352180:AAEK_mPLAl-S64EV1H7cWiNI2DmDr9pIZBk"   # التوكن الجديد
GROUP_ID   = -1003487080023    # ID متاع السوبر غروب متاع الفورمسيون
ADMIN_ID   = 6731717152        # الآدمن (إنت)
USERS_FILE = "users.json"

# وقت البقاء في الغروب قبل الطرد (بالثواني)
AUTO_KICK_SECONDS = 3 * 60   # 3 دقايق
...
# ========= مساعدة: إنشاء رابط دعوة + طرد بعد 3 دقايق =========
def create_one_time_link(user_id: int):
    invite = bot.create_chat_invite_link(
        chat_id=GROUP_ID,
        member_limit=1
    )
    # جدول طرد أوتوماتيك بعد وقت معيّن من لحظة دخولو للرابط
    schedule_auto_kick(user_id)
    return invite.invite_link


def schedule_auto_kick(user_id: int):
    def _kick():
        try:
            # نطردو من الغروب
            bot.ban_chat_member(GROUP_ID, user_id)
            # نعملو unban باش ينجم يرجع يدخل من جديد عبر البوت والموقع
            bot.unban_chat_member(GROUP_ID, user_id)
            bot.send_message(
                user_id,
                "⏱ انتهى وقت الجلسة (3 دقائق).\n"
                "لو تحب ترجع للفورماسيون، أطلب كود جديد من البوت وادخل من الموقع 👍"
            )
        except Exception:
            pass

    t = threading.Timer(AUTO_KICK_SECONDS, _kick)
    t.daemon = True
    t.start()
...
def handle_start(message):
    ...
    # رابط الموقع الصحيح متاعك في Render
    ...
    txt = (
        ...
        f"`{code}`\n\n"
        "✅ إدخل للموقع هذا وحط الكود باش تتحقق من جهازك:\n"
        f"{site_url}\n\n"
        "📌 ملاحظة: الدخول للجروب يكون من خلال الموقع فقط، وكل جلسة مدّتها 3 دقائق."
    )
...
def api_access():
    ...
    # أول جهاز: نسجّل بصمة + نسمح بالدخول
    if not user_info.get("fingerprint"):
        ...
        try:
            invite_link = create_one_time_link(user_id)
    ...
    # نفس الجهاز + verified → نسمح بالدخول مباشرة
...
def api_verify_otp():
    ...
    if not user_info.get("otp_required") or not user_info.get("otp"):
        return jsonify({"ok": False, "message": "❌ ما فماش OTP مطلوب حالياً. استعمل الكود فقط."})
    ...
    if otp_input != str(user_info["otp"]):
        ...
        if attempts >= 3:
            ...
            return jsonify({"ok": False, "message": "⛔ OTP خطأ 3 مرات. تم حظرك من النظام."})
        else:
            remaining = 3 - attempts
            return jsonify({"ok": False, "message": f"❌ OTP غير صحيح. محاولات متبقية: {remaining}."})

    # OTP صحيح → نقبل الجهاز الجديد
    current_fp = make_fingerprint(request)
    user_info["fingerprint"] = current_fp
    user_info["verified"] = True
    user_info["otp"] = None
    ...
    try:
        ...
        try:
            bot.send_message(ADMIN_ID, f"✅ المستخدم {user_id} ثبّت جهاز جديد بنجاح.")
        except:
            pass

        return jsonify({
            "ok": True,
            "message": "✅ تم قبول الجهاز الجديد. هذا رابط الدخول (مرة واحدة):",
            "invite_link": invite_link
        })
    except:
        return jsonify({"ok": False, "message": "✅ تم قبول OTP لكن صار مشكل في إنشاء رابط الدخول."})


# ========= تشغيل البوت في Thread داخل نفس Service =========
def run_bot():
